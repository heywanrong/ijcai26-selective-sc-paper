...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig1_oracle_retention.png}
\caption{Oracle TTC-gate strictly dominates always-SC on 12/12
experiments. The dashed line marks always-SC parity. Retention
$\in[109\%,174\%]$ across all cells and datasets.}
\label{fig:oracle}
\end{figure}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig2_pareto_pooled.png}
\caption{TTC-Gate Pareto curves, pooled GSM8K $+$ MATH-500
($n{=}400$ per cell). Gold star: oracle; red $\times$: always-greedy;
black $\times$: always-SC. Curves sweep $\tau$ across the (cost,
retention) plane. Logistic (12 features) outperforms conf\_only on
3/4 cells at pooled scale; MLP loses universally.}
\label{fig:pareto}
\end{figure*}
